...
const AdminCoupons = () => {
  ...
  const toggleActive = async (id, is_active) => {
    try {
      await couponAPI.updateCoupon(id, { is_active: !is_active });
      load();
    } catch {
      toast.error('Failed');
    }
            ...
            <tbody>
              {coupons.map((c) => (
                <tr key={c.id}>
                  ...
                  <td style={{ fontWeight: 600 }}>{c.type === 'percentage' ? `${c.value}%` : `Rs. ${c.value}`}</td>
                  <td style={{ fontSize: 13, color: 'var(--color-text-muted)' }}>Rs. {c.min_order_amount}</td>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', backdropFilter: 'blur(4px)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div className="admin-form-card" style={{ width: 500, maxWidth: '90vw' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24 }}>
              <h3 style={{ fontFamily: 'var(--font-display)', fontSize: 20 }}>Create Coupon</h3>
            ...
            <form onSubmit={handleCreate} style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
              ...
              <div className="form-group"><label>Type</label><select value={form.type} onChange={(e) => setForm({ ...form, type: e.target.value })}><option value="percentage">Percentage (%)</option><option value="fixed">Fixed Amount (Rs.)</option></select></div>
